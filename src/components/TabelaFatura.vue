  ...
  <ul class="collapsible acordion">
    <li>
      <div class="collapsible-header">
        <i class="material-icons">payment</i>Faturas
      </div>
      <div class="collapsible-body">
        <div class="box-fatura" v-for="fatura in faturas" :key="fatura">
          <div class="fatura z-depth-2">
            <span class="item"> <strong>Nº: </strong>{{ fatura.id }} </span>
            <br />
            <span class="item">
    ...
    async pagar(id) {
      this.api
        ...
        .catch((error) => {
          this.$emit("pagarFatura", {
            titulo: "Pagamento da fatura",
            mensagem: error.response.data.errors.errors,
          });
        });
    },
  },
